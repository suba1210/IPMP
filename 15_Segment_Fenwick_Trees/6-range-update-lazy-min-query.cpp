...
public:
    ...
    void update(int ind, int low, int high, int l, int r, int val){
        ...
        if(lazy[ind]!=0) {
            st[ind] += lazy[ind]; // number of nodes in the range of that node ( high - low + 1)
        ...
        // l low high r
        if(l <= low  && high <= r){
            st[ind] += val; // number of nodes in the range of that node ( high - low + 1)
...
int main(){
    vector<int> arr = {5, 4, 9, 8, 3, 1};
    int size = arr.size();

    SegmentTreeMin seg(size);
